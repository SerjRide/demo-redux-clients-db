...
const baseGenerator = (year = 18, profit = 'low', name = '10') => {
  ...
  fs.writeFile(`./src/demo/orders/orders_${name}.json`, JSON.stringify(arrOfOrders), (err) => {
    if (err) throw err;
    console.log(`Асинхронная запись файла orders_${name} завершена.`);
  });
